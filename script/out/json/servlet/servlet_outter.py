    ...
    def make(self,stru_conf):
        ...
        for tbl_item in stru_list:
            for tbl_name in tbl_item:
                ...
                recv = ""
                for x in tbl_list:
                    send = send +self.servlet_send_java(x)+"\n"
                    recv = recv+self.servlet_recv_java(x)+"\n"

                write_file("D:\\Project\\PythonProject\\breed\\breed_orice\\out\\servlet\\recv\\"+tbl_name+".java",recv.encode("utf-8"))
                write_file("D:\\Project\\PythonProject\\breed\\breed_orice\\out\\servlet\\send\\"+tbl_name+".java",send.encode("utf-8"))
        #print stru_conf.get_out("out_dir")
    def servlet_send_java(self,field):
        var_f = open(self.__main_config.get_tpldir()+"servlet\\send_java.tpl")
        ...
        return x
    def servlet_recv_java(self,field):
        var_f = open(self.__main_config.get_tpldir()+"servlet\\recv_java.tpl")
